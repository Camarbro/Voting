...
def home(request):
    return render(request, 'Voto/home.html')

class RegistrarPromovido(FormView):
    template_name = 'Voto/RegistrarPromovido.html'
    form_class = Registro
...
class RegistroPromotor(FormView):
    template_name = 'Voto/registro.html'
    ...
    def form_valid(self,form):
        ...
        p.no_promovidos = form.cleaned_data['no_promovidos']
        p.save()
        return super (RegistroPromotor, self).form_valid(form)

def ReportesUsuarios(request):
    return render(request ,'Voto/ru.html')

def ReportesEstadisticos(request):
    return render(request ,'Voto/re.html')

class Reporte_VotoSi(ListView):
    template_name = 'Voto/votosi.html'
    model = Promovido

class Reporte_VotoNo(ListView):
    template_name = 'Voto/votono.html'
    model = Promovido

class Reporte_Promotores(ListView):
    template_name = 'Voto/promotores.html'
    model = Promotor

class Reporte_Promovidos(ListView):
    template_name = 'Voto/promovidos.html'
    model = Promovido
